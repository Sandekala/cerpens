<template>
  <div class="container">
    <form @submit.prevent="handlePost" class="flex flex-col relative">
      <div>
        <input required v-model="form.title" type="text" name="title" id="title" class="focus:outline-none focus:ring-0 h-24 placeholder:font-bold block m-auto border-none placeholder-shown" placeholder="Input title here..." />
      </div>
      ...
      <div>
        <textarea required v-model="form.content" class="focus:outline-none focus:ring-0 block m-auto border-none max-w-full" name="content" id="content" cols="120" rows="10" placeholder="Input content here..."></textarea>
      </div>
...
const handlePost = async () => {
  ...
  axios
    ...
    .then(
      axios
        .post(`${import.meta.env.VITE_APP_BASE_URL}/cerpen`, {
